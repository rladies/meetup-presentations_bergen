...
```{r penguins_barchart, include = FALSE}
ggplot(penguins) +
  geom_bar(aes(island))
```

`r chunk_reveal("penguins_barchart", break_type = "auto", chunk_options = "warning = FALSE", font_size_code = "50%", widths = c(0.5, 0.45))`
...
```{r penguins_linechart, include = FALSE}
penguins %>%
  count(year, sex) %>%
...
`r chunk_reveal("penguins_linechart", break_type = "auto", chunk_options = "warning = FALSE", font_size_code = "50%", widths = c(0.5, 0.45))`
...
```{r penguins_heatmap, include = FALSE}
penguins %>%
...
`r chunk_reveal("penguins_heatmap", break_type = "auto", chunk_options = "warning = FALSE", font_size_code = "50%", widths = c(0.5, 0.45))`
...
```{r penguins_parallel, include = FALSE}
penguins %>%
  group_by(species) %>%
  summarise(
  ...
  ungroup() %>%
  ggplot(
    aes(x = measurement, y = rank)
  ) +
  geom_bump(aes(color = species, group = species))
```

`r chunk_reveal("penguins_parallel", break_type = "auto", chunk_options = "warning = FALSE", font_size_code = "50%", widths = c(0.5, 0.45))`

---

class: left, middle

## ...many other geoms and extensions: https://exts.ggplot2.tidyverse.org/gallery/
...
penguins_facet_interactive <- penguins_facet +
  geom_point_interactive(
    aes(
      color = species,
      shape = species,
      tooltip = paste0(sex, ", living on ", island)
    )
  )
girafe(ggobj = penguins_facet_interactive)
```
]
.pull-right[
```{r penguins_species2_interactive}
penguins_facet_interactive <- penguins_facet +
  geom_point_interactive(
    aes(
      color = species,
...
```{r penguins_species3, include = FALSE}
penguins_facet +
  scale_color_manual(
    values = c("darkorange",
               "darkorchid",
...
`r chunk_reveal("penguins_species3", break_type = "auto", chunk_options = "warning = FALSE", font_size_code = "50%", widths = c(0.5, 0.45))`
